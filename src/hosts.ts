 ...
 */
function probeDefaultKey(): string {
 ...
 */
export function parseSshConfig(text: string, defaultUser?: string): HostEntry[] {
  const entries: HostEntry[] = []
  let current: PartialHost | undefined
...
export class HostTable {
  ...
  private manual: HostEntry[]
  private sshCache: { mtimeMs: number; entries: HostEntry[] } | undefined

  constructor(opts?: { sshConfigPath?: string; storePath?: string }) {
  ...
  addManual(payload: {
    ...
    const entry: HostEntry = {
      ...
      auth: hasKey
        ? { kind: 'key', keyPath: payload.keyPath as string, ...(payload.passphrase ? { passphrase: payload.passphrase } : {}) }
  ...
  private sshEntries(): HostEntry[] {
    ...
    if (this.sshCache && this.sshCache.mtimeMs === mtimeMs) return this.sshCache.entries
  ...
  private loadStore(): HostEntry[] {
    ...
    try {
      ...
      if (data?.version !== 1 || !Array.isArray(data.hosts)) throw new Error('bad shape')
      return data.hosts.map((h) => ({ ...h, source: 'manual' as const }))
    } catch {
