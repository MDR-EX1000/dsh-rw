...
export interface ReadResult {
  ...
  totalLines: number
  startLine: number
  endLine: number // last line actually returned (startLine - 1 when empty)
  truncated: boolean // file exceeded maxBytes and was cut
}

type Stat = Awaited<ReturnType<SftpLike['stat']>>
type DirItem = Awaited<ReturnType<SftpLike['readdir']>>[number]

const DEFAULT_MAX_LINES = 2000
const MAX_LINES_CAP = 10000
const DEFAULT_MAX_BYTES = 2 * 1024 * 1024

function clampInt(value: number | undefined, dflt: number, min: number, max: number): number {
...
function typeOf(attrs: Stat): RemoteEntry['type'] {
...
export class RemoteFs {
  ...
  async list(path?: string): Promise<RemoteEntry[]> {
    ...
    if (!st.isDirectory()) throw new RwError('NOT_A_DIRECTORY', `not a directory: ${p}`)
    let items: DirItem[]
    try {
   ...
   */
  async read(path: string, opts?: { startLine?: number; maxLines?: number; maxBytes?: number }): Promise<ReadResult> {
    const p = resolveInWorkspace(this.root, path)
    await assertRealpathInside(this.sftp, this.root, p)
    const st = await this.statChecked(p, 'stat')
    if (st.isDirectory()) throw new RwError('NOT_A_DIRECTORY', `is a directory: ${p}`)
  ...
  async write(path: string, content: string | Buffer, opts?: { mkdir?: boolean }): Promise<{ bytes: number }> {
    const p = resolveInWorkspace(this.root, path)
    await assertWritableInside(this.sftp, this.root, p)
    const data = typeof content === 'string' ? Buffer.from(content, 'utf8') : content
    if (opts?.mkdir ?? true) await this.mkdirp(dirName(p))
    try {
  ...
  async mkdir(path: string): Promise<void> {
    ...
    if (p === this.root) await assertRealpathInside(this.sftp, this.root, p)
    else await assertWritableInside(this.sftp, this.root, p)
    await this.mkdirp(p)
  }
   ...
   */
  async move(src: string, dst: string, opts?: { overwrite?: boolean }): Promise<void> {
    ...
    if (srcStat.isSymbolicLink()) await assertWritableInside(this.sftp, this.root, s)
    else await assertRealpathInside(this.sftp, this.root, s)
    await assertWritableInside(this.sftp, this.root, d)

    let dstStat: Stat | undefined
    try {
    ...
    if (dstStat) {
      ...
      try {
        if (dstStat.isDirectory()) await this.sftp.rmdir(d)
        else await this.sftp.unlink(d)
      } catch (err) {
  ...
  async stat(path: string): Promise<{ type: RemoteEntry['type']; size: number; mtime: number }> {
    const p = resolveInWorkspace(this.root, path)
    await assertRealpathInside(this.sftp, this.root, p)
    const st = await this.statChecked(p, 'stat')
  ...
  private async statChecked(p: string, how: 'stat' | 'lstat'): Promise<Stat> {
    try {
  ...
  private async unlinkChecked(p: string): Promise<void> {
    try {
  ...
  private async deleteChildren(dir: string): Promise<void> {
    let items: DirItem[]
    try {
  ...
  private async mkdirp(p: string): Promise<void> {
    ...
    try {
      ...
      if (!st.isDirectory()) throw new RwError('INVALID_INPUT', `exists and is not a directory: ${p}`)
      exists = true
    } catch (err) {
