...
const PLACEHOLDER_NOTE = 'placeholder only — not a copy of remote files';
 ...
 */
export function resolvePlaceholderDir(alias, remotePath, baseDir) {
    ...
    for (const name of names) {
        ...
        if (meta !== null && meta.alias === alias && meta.remotePath === normalized)
            return dir;
    }
    return null;
}
 ...
 */
function freshPlaceholderDir(alias, normalized, baseDir, displayName) {
    const parent = join(baseDir ?? defaultBaseDir(), sanitize(alias));
 ...
 */
export function ensurePlaceholder(alias, entry, remotePath, baseDir, displayName) {
    ...
    const consistent = existing !== null &&
...
export function readPlaceholderMeta(dir) {
    let raw;
    try {
 ...
 */
export function findPlaceholderByPath(cwd, baseDir) {
    ...
    for (const aliasName of aliasDirNames) {
        ...
        for (const name of placeholderNames) {
            ...
            if (meta === null)
                continue;
            const dirRoots = [resolve(dir)];
            try {
                const real = realpathSync(dir);
                if (!dirRoots.includes(real))
                    dirRoots.push(real);
            }
            catch {
