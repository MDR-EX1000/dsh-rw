 ...
 */
export async function expandRemoteHome(sftp, p) {
    if (p !== '~' && !p.startsWith('~/'))
        return p;
    let home;
    try {
 ...
 */
export async function assertRealpathInside(sftp, root, p) {
    const r = normalizeRemote(root);
    let real;
    try {
 ...
 */
export async function assertWritableInside(sftp, root, p) {
    const r = normalizeRemote(root);
    let cur = dirName(p);
    for (;;) {
        if (!inside(r, cur)) {
            throw new RwError('NO_SUCH_PATH', `no existing ancestor of ${p} inside workspace ${r} (does the workspace root exist?)`);
        }
        let real;
        try {
