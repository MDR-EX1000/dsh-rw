            ...
            handler: async (req, res) => {
                ...
                if (method === 'DELETE') {
                    ...
                    if (session.alias === alias)
                        session.set({ alias: null, workspace: null });
        ...
        // --------------------------------------------------- GET /api/dsh-rw/ls
        {
            ...
            handler: async (req, res) => {
                ...
                try {
                    ...
                    if (!isDir)
                        throw new RwError('NOT_A_DIRECTORY', `not a directory: ${dir}`);
                    let items;
                    try {
                    ...
                    const entries = items
                        ...
                        .map((it) => ({
                        name: it.filename,
                        type: (it.attrs.isSymbolicLink() ? 'symlink' : it.attrs.isDirectory() ? 'dir' : 'file'),
                    }))
